    ...
    public function boot()
    {
       try {
           $shurjopay = PaymentGateway::where(['status' => 1, 'type' => 'shurjopay'])->first();
            ...
            $sidecategories = Category::where('parent_id','=','0')->where('status',1)->select('id','name','slug','status')->get();
            view()->share('sidecategories',$sidecategories); 
            
            $menucategories = Category::where('status',1)->select('id','name','slug','status')->get();
            ...
            $footers = Footer::where('status',1)->orderBy('sort_order','asc')->get();
           ...
           // Ignore errors during migrations or if columns don't exist
       }
    }
}
