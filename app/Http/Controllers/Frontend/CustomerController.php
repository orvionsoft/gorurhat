...
class CustomerController extends Controller
{
    ...
    public function signin(Request $request){
        ...
        if($auth_check){
            ...
            Toastr::error('message', 'Opps! your email or password wrong');
            return redirect()->back();
        }else{
    ...
    public function forgot_verify(Request $request){
        ...
        if(!$customer_info){
            Toastr::error('Your email address not found');
            return back();
        }
        $customer_info->forgot = rand(1111,9999);
        $customer_info->save();
        
        // Send email with OTP using Mailable class
        try {
            Mail::to($customer_info->email)->send(new ForgotPasswordOTP($customer_info));
        } catch (\Exception $e) {
    ...
    public function forgot_resend(Request $request){
        $customer_info = Customer::where('email',session::get('verify_email'))->first();
        $customer_info->forgot = rand(1111,9999);
        $customer_info->save();
        ...
        Toastr::success('Success','OTP resent to your email');
        return redirect()->back();
    }
    ...
    public function forgot_store(Request $request){
        ...
        $customer_info->save();
        if(Auth::guard('customer')->attempt(['email' => $customer_info->email, 'password' => $request->password])) {
    ...
    public function checkout(Request $request){
       ...
       return view('frontEnd.layouts.customer.checkout',compact('shippingcharge', 'bkash_gateway', 'shurjopay_gateway','order'));
    }
    public function order_save(Request $request){
        ...
        if(Auth::guard('customer')->user()){
            ...
            $exits_customer = Customer::where('email',$request->email)->select('email','id')->first();
            if($exits_customer){
