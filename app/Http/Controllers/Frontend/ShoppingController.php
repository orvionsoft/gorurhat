...
class ShoppingController extends Controller
{
    ...
    // Cow Cart Methods
    public function addCowToCartGet($id, Request $request)
    {
        $qty = 1;
        $cowInfo = DB::table('cows')->where('id', $id)->first();
        $cowImage = DB::table('cow_images')->where('cow_id', $id)->first();
        
        if (!$cowInfo) {
            return response()->json(['success' => false, 'message' => 'Cow not found.']);
        }

        $image = $cowImage ? $cowImage->image : 'uploads/default.png';
    ...
    public function addCowToCartPost(Request $request)
    {
        $cow = Cow::where(['id' => $request->id])->first();
        
        if (!$cow) {
            Toastr::error('Cow not found', 'Error!');
            return redirect()->back();
        }

        $cowImage = $cow->image();
        $image = $cowImage ? $cowImage->first()->image : 'uploads/default.png';
        ...
        Toastr::success('Cow successfully added to cart', 'Success!');
