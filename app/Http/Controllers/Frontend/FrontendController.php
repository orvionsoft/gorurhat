...
class FrontendController extends Controller
{
    public function index()
    {
        ...
        $footertopads = Banner::where(['status' => 1, 'category_id' => 6])
         ...
         $flas_sales = Product::where(['status' => 1, 'flashsale' => 1])
            ->orderBy('id', 'DESC')
            ->select('id', 'name', 'slug', 'new_price', 'old_price','sold','stock', 'weight', 'category_id')
            ->with('prosizes', 'procolors', 'category', 'image')
            ->limit(12)
            ->get();
        // return $hotdeal_top;
        ...
        $hotdeal_top = Product::where(['status' => 1, 'topsale' => 1])
            ->orderBy('id', 'DESC')
            ->select('id', 'name', 'slug', 'new_price', 'old_price','stock', 'weight', 'category_id')
            ->with('prosizes', 'procolors', 'category', 'image')
            ->limit(12)
            ->get();
        // return $hotdeal_top;
        ...
        $offerCategories = Category::where(['status' => 1, 'front_view' => 1])
            ->orderBy('id', 'ASC')
            ->with(['products' => function ($query) {
                $query->where('status', 1)
                    ->select('id', 'name', 'slug', 'category_id', 'new_price', 'old_price', 'sold', 'stock', 'weight')
                    ->orderBy('id', 'DESC')
                    ->limit(8);
            }, 'products.image'])
            ->get();
        ...
        if ($offerCategories->isEmpty()) {
            $offerCategories = Category::where('status', 1)
                ->orderBy('id', 'ASC')
                ->with(['products' => function ($query) {
                    $query->where('status', 1)
                        ->select('id', 'name', 'slug', 'category_id', 'new_price', 'old_price', 'sold', 'stock', 'weight')
        ...
        $cows = Cow::where('status', 'active')
            ->select('id', 'name', 'slug', 'price', 'breed', 'age', 'weight', 'color')
            ->with('image', 'images')
            ->latest()
            ->limit(12)
            ->get();
        
        return view('frontEnd.layouts.pages.index', compact('sliders', 'frontcategory', 'hotdeal_top', 'hotdeal_bottom', 'homeproducts', 'sliderbottomads', 'footertopads','flas_sales','campaognads','reviews','all_products','offerCategories','cartData','cows'));
    }
    ...
    public function shop(Request $request)
    {
        ...
        return view('frontEnd.layouts.pages.shop', compact('products', 'categories', 'min_price', 'max_price', 'offerCategories'));
    }

    public function cows(Request $request)
    {
        $cows = Cow::where(['status' => 'active'])
            ->select('id', 'name', 'slug', 'price', 'breed', 'age', 'weight', 'color')
            ->with('image', 'images');
        ...
        if ($request->sort == 1) {
            $cows = $cows->orderBy('created_at', 'desc');
        } elseif ($request->sort == 2) {
            $cows = $cows->orderBy('created_at', 'asc');
        } elseif ($request->sort == 3) {
            $cows = $cows->orderBy('price', 'desc');
        } elseif ($request->sort == 4) {
            $cows = $cows->orderBy('price', 'asc');
        } elseif ($request->sort == 5) {
            $cows = $cows->orderBy('name', 'asc');
        } elseif ($request->sort == 6) {
            $cows = $cows->orderBy('name', 'desc');
        } else {
            $cows = $cows->latest();
        }

        $min_price = Cow::where('status', 'active')->min('price');
        $max_price = Cow::where('status', 'active')->max('price');

        if ($request->min_price && $request->max_price) {
            $cows = $cows->where('price', '>=', $request->min_price);
            $cows = $cows->where('price', '<=', $request->max_price);
        }

        $cows = $cows->paginate(21);
        return view('frontEnd.layouts.pages.cows', compact('cows', 'min_price', 'max_price'));
    }
