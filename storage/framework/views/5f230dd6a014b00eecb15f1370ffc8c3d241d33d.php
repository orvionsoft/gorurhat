...
<section style="
    ...
    background-image: url('<?php echo e(asset($sliders->first()->image ?? '')); ?>'); 
    ...
    <div style="flex: 1; padding-right: 40px;">
        ...
        <div style="display: flex; gap: 15px; flex-wrap: wrap;">
            <a href="<?php echo e(route('shop')); ?>" style="background-color: #d30000; color: white; border: none; padding: 8px 18px; border-radius: 8px; cursor: pointer; font-size: 14px; display: inline-flex; align-items: center; text-decoration: none;">
...
<section style="padding: 60px 20px; text-align: center; font-family: sans-serif; background-color: #f9f9f9;">
    ...
    <div style="display: flex; justify-content: center; flex-wrap: wrap; gap: 30px; max-width: 1200px; margin: 0 auto;">
        ...
        <div style="flex: 1; min-width: 200px; max-width: 250px;">
            <a href="<?php echo e(route('shop')); ?>" style="text-decoration: none; color: inherit; display: block;">
                        ...
                        <div class="product_item wist_item">
                            <div class="product_item_inner">
                                ...
                                <div class="pro_des">
                                    ...
                                    <span style="background-color:#FFBCA5" class="px-3 py-0 rounded-pill">Sold <?php echo e($value->sold??0); ?></span>
        ...
        <div id="offer-more-slider" style="display: flex; gap: 20px; padding: 10px 0; overflow-x: hidden; scroll-behavior: smooth; scrollbar-width: none;">
            ...
            <div class="product-card" style="box-shadow: 0 8px 30px rgba(0,0,0,0.1); background: white; border-radius: 12px;  box-shadow: 0 4px 15px rgba(0,0,0,0.08); height: 350px; display: flex; flex-direction: column; min-width: 250px; flex-shrink: 0; transition: all 0.3s ease;">
                <div class="product-card-image" style="position: relative; height: 180px;">
                    <a href="<?php echo e(route('product', $value->slug)); ?>">
                        <img src="<?php echo e(asset($value->image ? $value->image->image : 'https://via.placeholder.com/300x200')); ?>" alt="<?php echo e($value->name); ?>" style="width: 100%; height: 100%; object-fit: cover; transition: transform 0.3s ease;">
                    </a>
                    <div class="product-badge" style="position: absolute; top: 12px; left: 12px; background: rgba(179, 0, 0, 0.9); color: #fff; padding: 6px 10px; border-radius: 999px; font-size: 12px;">
                        ● <?php echo e($value->category->name ?? 'প্রিমিয়াম'); ?>
                ...
                <div class="product-card-body" style="padding: 15px; display: flex; flex-direction: column; flex: 1;">
                    <h3 style="margin: 0 0 10px 0; font-size: 18px; color: #333; white-space: nowrap; overflow: hidden; text-overflow: ellipsis;"><?php echo e(Str::limit($value->name, 30)); ?></h3>
                    <div class="product-details" style="display: flex; justify-content: space-between; gap: 10px; color: #777; font-size: 14px; margin-bottom: 15px;">
                        <span>⚖ <?php echo e(is_numeric($value->weight) ? $value->weight . ' কেজি' : ($value->weight ?? 'ওজন নেই')); ?></span>
                        <span style="margin-right: 50px">🎗 <?php echo e($value->category->name ?? 'প্রিমিয়াম'); ?></span>
                    </div>
                    <div class="product-footer" style="display: flex; justify-content: space-between; align-items: center; margin-top: auto; gap: 10px;">
                        <span class="product-price" style="color: #b30000; font-weight: 700; font-size: 18px;">৳ <?php echo e(number_format($value->new_price, 0)); ?></span>
                        <a href="<?php echo e(route('product', $value->slug)); ?>" class="product-btn" style="color: #b30000; text-decoration: none; font-weight: 500; padding: 5px 10px; background: #ffeaea; border-radius: 5px; transition: all 0.3s ease;">
                            বিস্তারিত দেখুন →
                        </a>
                    </div>
                </div>
...
<section style="padding: 60px 5%; font-family: 'Segoe UI', Arial, sans-serif; text-align: center; color: #333;">
    ...
    <?php $offerProductCount = $offerCategories->sum(fn($category) => $category->products->count()); ?>

    <?php if($offerCategories->count()): ?>
    ...
    <?php $__currentLoopData = $offerCategories; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $category): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
        <button class="tab-btn" onclick="filterSelection('<?php echo e($category->slug); ?>', this)" style="background-color: transparent; color: #333; border: none; padding: 10px 25px; border-radius: 35px; cursor: pointer; font-weight: 500; transition: all 0.3s ease;" onmouseover="this.style.backgroundColor='#ffebeb'; this.style.color='#d30000'; this.style.transform='translateY(-2px)';" onmouseout="this.style.backgroundColor='transparent'; this.style.color='#333'; this.style.transform='translateY(0)';"><?php echo e($category->name); ?></button>
    <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
</div>
        ...
        <div id="product-container" class="offer-products-grid">
            <?php $__currentLoopData = $offerCategories; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $category): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                <?php $__currentLoopData = $category->products; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $product): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                    <div style="box-shadow: 0 8px 30px rgba(0,0,0,0.1); height: 350px;" class="filter-item <?php echo e($category->slug); ?> product-card">
                        <div class="product-card-image">
                            <img src="<?php echo e(asset($product->image->image ?? 'public/frontEnd/images/placeholder.png')); ?>" alt="<?php echo e($product->name); ?>">
                            <div class="product-badge"><?php echo e($category->name); ?></div>
                            <?php if($product->stock < 1): ?>
                                <div class="stock-out-overlay">STOCK OUT</div>
                            <?php endif; ?>
                        </div>
                        <div class="product-card-body">
                            <h3><?php echo e(Str::limit($product->name, 45)); ?></h3>
                            <div class="product-details">
                                <span>⚖ <?php echo e(is_numeric($product->weight) ? $product->weight . ' কেজি' : ($product->weight ?? 'ওজন নেই')); ?></span>
                                <span style="margin-right: 30px;">🎗 <?php echo e($category->name); ?></span>
                            </div>
                            <div class="product-footer">
                                <span class="product-price">৳ <?php echo e(number_format($product->new_price)); ?></span>
                                <a href="<?php echo e(route('product', $product->slug)); ?>" class="product-btn">বিস্তারিত <i class="bi bi-arrow-right"></i></a>
                            </div>
                        </div>
                    </div>
                <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
        ...
        <?php if($offerProductCount > 8): ?>
            <div style="margin-top: 30px; text-align: center;">
                <a href="<?php echo e(route('shop')); ?>" style="display: inline-block; z-index: 1; background: #d30000; color: #fff; padding: 14px 28px; border-radius: 50px; font-size: 16px; text-decoration: none; transition: background 0.3s;">আরও দেখুন</a>
            </div>
        <?php endif; ?>
    <?php else: ?>
