...
<div class="qurbani-header">
    ...
    <div class="qurbani-products">
        @foreach($products->take(4) as $value)
        <div class="product-card">
            <div class="product-card-image">
                <img src="{{ asset($value->image ? $value->image->image : 'https://via.placeholder.com/300x200') }}" alt="{{$value->name}}">
                <div class="product-badge">
                    ● {{ $value->category->name ?? 'ক্যাটাগরি নেই' }}
                </div>
                @if($value->stock < 1)
            ...
            <div class="product-card-body">
                <h3>{{Str::limit($value->name, 30)}}</h3>
                <div class="product-details">
                    <span>⚖ {{ $value->weight ? $value->weight.' কেজি' : 'ওজন নেই' }}</span>
                    <span>🎗 {{ $value->category->name ?? 'নেই' }}</span>
                </div>
                <div class="product-footer">
                    <span class="product-price">৳ {{number_format($value->new_price, 0)}}</span>
                    <a href="{{ route('product',$value->slug) }}" class="product-btn">বিস্তারিত দেখুন →</a>
                </div>
            </div>
        </div>
        @endforeach
    ...
    <div class="qurbani-filter">
        <form action="{{ route('shop') }}" method="GET" id="filterForm" style="display: contents; width: 100%;">
            ...
            <div class="filter-group">
                <div class="range-label">
                    ...
                    <span class="weight-inputs">
                        <input type="number" name="min_weight" placeholder="সর্বনিম্ন" value="{{ request('min_weight') }}" id="minWeightInput">
                        <span>-</span>
                        <input type="number" name="max_weight" placeholder="সর্বোচ্চ" value="{{ request('max_weight') }}" id="maxWeightInput">
                    </span>
                </div>
            ...
            <div class="filter-group">
                <div class="range-label">
                    ...
                    <span class="price-inputs">
                        <input type="number" name="min_price" placeholder="সর্বনিম্ন" value="{{ request('min_price') }}" id="minPriceInput">
                        <span>-</span>
                        <input type="number" name="max_price" placeholder="সর্বোচ্চ" value="{{ request('max_price') }}" id="maxPriceInput">
                    </span>
                </div>
        ...
        <div class="row">
            <div class="col-sm-12">
                <div class="category-product main_product_inner" style="display: grid; grid-template-columns: repeat(4, 1fr); gap: 20px;">
                    ...
                    <div style="box-shadow: 0 8px 30px rgba(0,0,0,0.1); height: 350px;" class="filter-item {{ $value->category->slug ?? '' }} product-card">
                        <div class="product-card-image">
                            <img src="{{ asset($value->image->image ?? 'public/frontEnd/images/placeholder.png') }}" alt="{{ $value->name }}">
                            <div class="product-badge">{{ $value->category->name ?? 'ক্যাটাগরি নেই' }}</div>
                            @if($value->stock < 1)
                                <div class="stock-out-overlay">STOCK OUT</div>
                            @endif
                        </div>
                        <div class="product-card-body">
                            <h3>{{ Str::limit($value->name, 45) }}</h3>
                            <div class="product-details">
                                <span>⚖ {{ $value->weight ?? 'ওজন নেই' }} কেজি</span>
                                <span style="margin-right: 20px;">🎗 {{ $value->category->name ?? 'নেই' }}</span>
                            </div>
                            <div class="product-footer" style="padding-top: 20px">
                                <span class="product-price">৳ {{ number_format($value->new_price) }}</span>
                                <a href="{{ route('product', $value->slug) }}" class="product-btn">বিস্তারিত দেখুন →</a>
                            </div>
                        </div>
                    </div>
